...
### Agent Tools (MCP) — let other agents use TradingSpy functions

With `ENABLE_AGENT_TOOLS=true`, external agents (opencode, Hermes, any MCP or OpenAI function-calling client) can call TradingSpy's deterministic functions directly — market data, quotes, news, technicals, insider trades, fundamental screening and backtests — while the remote agent keeps using **its own LLM** for reasoning. Tools are read-only/compute-only: no deletes, no resets.

`render_chart_image` additionally returns **branded TradingSpy PNG charts** (candles, expected-pattern forecast, or pattern detection) as MCP image content — the backend analog of the web UI's "Generated by TradingSpy" footer.

| Surface | Endpoint |
| --- | --- |
| MCP server (JSON-RPC) | `POST http://localhost:8000/mcp` |
| OpenAI tool manifest | `GET http://localhost:8000/api/tools/manifest` |
| Generic invoke | `POST http://localhost:8000/api/tools/invoke` |
...
Full tool list and usage examples: [skills/skills.md](skills/skills.md).
