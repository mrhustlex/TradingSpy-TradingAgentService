...
const cleanupLocalStorage = () => {
    ...
    // or point to wrong ports (8001, 8003, etc.)
    if (dataUrl && (!dataUrl.includes('/api/') || !dataUrl.includes(':8000'))) {
        console.log('Cleaning up incorrect DATA_SERVICE URL from localStorage:', dataUrl);
        localStorage.removeItem('setting_data_url');
    }
    if (backtestUrl && (!backtestUrl.includes('/api/') || !backtestUrl.includes(':8000'))) {
    ...
    if (optimizerUrl && (!optimizerUrl.includes('/api/') || !optimizerUrl.includes(':8000'))) {
    ...
    if (intelligenceUrl && (!intelligenceUrl.includes('/api/') || !intelligenceUrl.includes(':8000'))) {
